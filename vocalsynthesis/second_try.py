...
window_shift = 1000
batch_size = 100
truncated_BPTT_length = 100 # is this the same as seq length? Is using variable length sequences the same as variable length tbptt?
secs_to_generate = 60
...
print 'window shift: '+str(window_shift)
dataset_name = datafile.split('.')[0]+'_' + str(input_length)+'_'+str(target_length)+'_'+str(seq_length)+'_'+str(window_shift)
full_dataset = []
for what_dataset,dataset in enumerate(train_test_val):
    ...
    current_length = 0
    while not current_length > len(dataset) - required_length
